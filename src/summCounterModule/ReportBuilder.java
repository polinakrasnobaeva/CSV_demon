...
public class ReportBuilder{
	...
	public DOCXworker buildReport(String docExamplePath){
		...
		AppWindow.printToLocalConsole("Конвертация таблиц успешно завершена");
		AppWindow.printToLocalConsole("ВСЕГО ФРАЗ: " + this.totalPhraseCount);
		AppWindow.printToLocalConsole("В ТОП ФРАЗ: " + this.topPhraseCount);
		return dw;
	}
	...
	private void doRostov(boolean isNeed){
		...
		if(eof)
			AppWindow.printToLocalConsole("Ростов указан в отчете, но файл внезапно закончился((");
			...
			while(!csvReader.isTableSeparator(currentLine.getString())){ //пока следующая строка не разделитель
				...
				if(eof){
					if(i == 3){
						...
						AppWindow.printToLocalConsole("Не хватает таблиц по Ростову, или они не разделены");
					}
				}
			}
		...
		if(isNeedTableChecking){
			if(this.singlesForEtalon != null){
				...
				AppWindow.printToLocalConsole("Опция проверки целостности включена, но не было бонуса");
			}
		}
	...
	private void doCities(boolean isNeed){
		...
		if(eof) 
			AppWindow.printToLocalConsole("Другие города указан в отчете, но файл внезапно закончился((");
		...
		while(!eof){
			...
			if(eof)
				AppWindow.printToLocalConsole("Внезапный конец файла при просмотре других городов");
		...
		if(!isAddWords & oneCityTables.size() % 2 == 1){
			AppWindow.printToLocalConsole("В настройках указано отсутствие доп. слов, а таблиц по доп. городам - нечетное кол-во.\nЧто-то тут не так, проверь");
		}
		...
		for(int i = 0; i < oneCityTables.size(); i+=2){
			if(i+1 < oneCityTables.size()){
				...
				AppWindow.printToLocalConsole(city1add + ";" + city2add);
				...
				if(m1.matches() | m2.matches()){ //если один из регэкспов совпал, значит, окончания - города, у одного в именит., у другого в предложном падеже с предлогом в|во
					...
					AppWindow.printToLocalConsole("Найдены доп. слова!");
					break;
				}
		...
		if(realCities.isEmpty()){
			AppWindow.printToLocalConsole("Дополнительных городов не было найдено!");
		}else{
