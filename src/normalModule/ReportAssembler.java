...
public class ReportAssembler {
	...
	public static void assebmle(String sourceFileName, WordprocessingMLPackage WMLPTables, String sourceMetricaFileName, String resultFileName,
		...
		string.setValue(values.get("phphonenumber"));
		//вставляем эл. почту
		Hyperlink link = createHyperlink(doc, values.get("phmailto"));
		...
		for (Iterator<Object> iterator = objectList.iterator(); iterator.hasNext();) {
			...
			for (Object text : textElements) {
				...
				if (textElement.getValue() != null && textElement.getValue().equals(phText)){
					AppWindow.printToLocalConsole("Текст параграфа с плейсхолдером " + phText + " нашелся");
					return textElement;
				}
			}
		}
		AppWindow.printToLocalConsole("Параграф с плейсхолдером " + phText + " не нашелся");
		return null;
	}
	...
	private static P findByPlaceholderParagraph(List<Object> objectList, String phText) throws Docx4JException, JAXBException {
		for (Iterator<Object> iterator = objectList.iterator(); iterator.hasNext();) {
			...
			for (Object text : textElements) {
				Text textElement = (Text) text;
				if (textElement.getValue() != null && textElement.getValue().equals(phText)){
					AppWindow.printToLocalConsole("Параграф с плейсхолдером " + phText + " нашелся");
					return (P) obj;
				}
			}
		}
	...
	private static void writeDocx(WordprocessingMLPackage doc, String saveFilePath){
		...
		try {
			...
			AppWindow.printToLocalConsole("Не удалось сохранить в файл");
		}
		AppWindow.printToLocalConsole("НАПЕЧАТАЛИ В ДОК");
	}
